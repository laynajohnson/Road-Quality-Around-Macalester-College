...
```{r}
feb19 <- process_and_plot(feb19, "February 19th")
feb27 <- process_and_plot(feb27, "February 27th")
mar06 <- process_and_plot(mar06, "March 6th")
mar13 <- process_and_plot(mar13, "March 13th")
mar20 <- process_and_plot(mar20, "March 20th")
mar27 <- process_and_plot(mar27, "March 27th")
apr03 <- process_and_plot(apr03, "April 3rd")
```
...
L2_norm_table <- function(df) {
  df %>%
    group_by(base_street) %>%
    summarise(
      times_uncomfortable = sum(L2_norm > 1.6, na.rm = TRUE),
      max_L2_norm = max(L2_norm, na.rm = TRUE),
      avg_disc = mean(L2_norm, na.rm = TRUE)
    ) 
  }
